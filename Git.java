...
public class Git implements GitInterface {
    ...
    public void stage(String filePath) throws IOException {
        ...
        if (stagingFile.exists()) {
            ...
            removeEntry(entries, stagePath);
            // Update the index
            updateIndex();
            // Rebuild the tree to reflect the deletion
            Blob.addTree(root.getPath(), root.getName());
        }
    }

    public static void removeEntry(ArrayList<String> entries, String filePath) throws IOException {
        for (int i = 0; i < entries.size(); i++) {
            String entry = entries.get(i);
            int indexOfFilePath = entry.indexOf(filePath);
            if (indexOfFilePath > -1 && indexOfFilePath + filePath.length() == entry.length()) {
                entries.remove(i);
    ...
    // @param filePath the file path that would appear in the log for the hashed
    // file
    ...
    // @return boolean true if entry was successful changed, false if filepath was
    // not found or newHash is the same as the old hash
    public static boolean setHashInEntry(String newHash, String filePath) throws IOException {
        for (int i = 0; i < entries.size(); i++) {
            ...
            if (indexOfFilePath > 0 && !oldHash.equals(newHash)
                    && indexOfFilePath + filePath.length() == entryString.length()) {
    ...
    public void checkRemovedFiles() {
        for (String entry : entries) {
            // Entry format: "[type] [hash] [filePath]"
            int firstSpace = entry.indexOf(' ');
            int secondSpace = entry.indexOf(' ', firstSpace + 1);
            if (firstSpace > -1 && secondSpace > -1) {
                String type = entry.substring(0, firstSpace);
                String hash = entry.substring(firstSpace + 1, secondSpace);
                String filePath = entry.substring(secondSpace + 1);
                File file = new File(filePath);
                if (!file.exists()) {
                    System.out.println("File removed: SHA: " + hash);
                }
            }
        }
    }
